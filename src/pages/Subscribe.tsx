      ...
      <div className="w-full max-w-[1100px] flex items-center justify-between mt-20 mx-auto">
        ...
        <div className="p-8 bg-gray-700 border border-gray-500 rounded">
          ...
          <form onSubmit={handleSubscribe} className="flex flex-col gap-2 w-full" action="">
            <input className="bg-gray-900 rounded px-5 h-14" onChange={event => setName(event.target.value)} type="text" placeholder="seu nome completo" />
            <input className="bg-gray-900 rounded px-5 h-14" onChange={event => setEmail(event.target.value)} type="text" placeholder="digite seu email" />
            <button disabled={loading} className="mt-4 bg-green-500 uppercase py-4 rounded font-bold hover:bg-green-700 transition-colors disabled:opacity-50" type="submit">Garantir minha vaga</button>
          </form>
        </div>
      </div>
